    ...
    def scan(self, url: str) -> list[dict[str, Any]]:
        ...
        for param_name, original_value in params:
            for payload in SAFE_XSS_PAYLOADS:
                test_params = [(name, payload if name == param_name else value) for name, value in params]
                ...
                vulnerable = reflected_raw and payload.startswith('<')
                evidence = 'Payload not reflected'
                if reflected_raw:
                    evidence = 'Raw payload reflected in response'
                elif reflected_encoded:
                    evidence = 'Payload reflected after HTML encoding'
                elif dom_context:
                    evidence = dom_context
                findings.append({'parameter': param_name, 'original_value': original_value, 'payload': payload, 'vulnerable': vulnerable, 'evidence': evidence, 'severity': 'Medium' if vulnerable else 'Low', 'sanitized': reflected_encoded and (not reflected_raw)})
        return findings
