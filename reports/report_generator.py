...
class ReportGenerator:
    ...
    def save_txt(self, results: dict[str, Any], base_name: str) -> Path:
        ...
        lines = ['MINI PENETRATION TESTING TOOLKIT REPORT', f"Target URL: {results['target_url']}", f"Scan Time: {results['scan_time']}", f"Risk Score: {results['risk_score']}/100 ({results['risk_severity']})", '', 'URL Scan:', json.dumps(results.get('url_scan', {}), indent=2), '', 'Port Scan:', tabulate(results.get('port_scan', []), headers='keys'), '', 'SQL Injection:', tabulate(results.get('sql_injection', []), headers='keys'), '', 'XSS:', tabulate(results.get('xss', []), headers='keys'), '', 'SSL:', json.dumps(results.get('ssl', {}), indent=2), '', 'Recommendations:', '\r\n'.join((f'- {item}' for item in results.get('recommendations', [])))]
    ...
    def save_pdf(self, results: dict[str, Any], base_name: str) -> Path | None:
        ...
        story: list[Any] = []
        story.append(Paragraph('Mini Penetration Testing Toolkit Report', styles['Title']))
